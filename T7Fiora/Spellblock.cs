...
namespace T7_Fiora 
{
    public static class SpellBlock
    {
        ...
        {
            if (!Program.check(Program.blocking,"BLOCK") || !DemSpells.W.IsReady()) return;

            foreach (var skillshot in Evade.Evade.GetSkillshotsAboutToHit(Player.Instance, (int)(DemSpells.W.CastDelay + Game.Ping / 2f)))
            {
                ...
                foreach (var spell in spells)
                {
                    ...
                    {
                        if (skillshot.Unit == null || !skillshot.Unit.IsValidTarget(DemSpells.W.Range))
                             DemSpells.W.Cast(Player.Instance.Position);

                        var cast = DemSpells.W.GetPrediction(skillshot.Unit);
                        var castPos = DemSpells.W.IsInRange(cast.CastPosition) ? cast.CastPosition : skillshot.Unit.ServerPosition;

                        DemSpells.W.Cast(castPos);
                    }
                }
            }
        }
