...
namespace T7_Fizz 
{
    public static class SpellBlock
    {
        ...
        {
            if (!Program.check(Program.blocking,"BLOCK") || !DemSpells.E.IsReady()) return;

            foreach (var skillshot in Evade.Evade.GetSkillshotsAboutToHit(Player.Instance, (int)(DemSpells.E.CastDelay + Game.Ping / 2f)))
            {
                if (!DemSpells.E.IsReady())
                    return;
